...
import { Activity, BarChart3, Clock, Database, Download, Eye, FileText, Tag, X, RefreshCw } from 'lucide-react'
import {
  BarChart,
  Bar,
  XAxis,
...
export default function PMClaims() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((err) => {
        console.error('Error loading PM claims:', err)
        setClaims([])
        setScreenshots([])
      })
  ...
  const handleExportCsv = async () => {
    ...
    try {
      ...
      if (!blob || blob.size === 0) {
        alert('Export returned no data. Ensure you have claims and try Refresh first.')
        return
      }
      ...
      alert(e?.message || 'Failed to export. See console for details.')
    } finally {
      setExporting(false)
    }
  }
  ...
  const handleReprocess = async () => {
    ...
    try {
      ...
      // Backend returns 202 and runs OCR in background to avoid gateway timeout / CORS errors
      if (result.started || result.message) {
        alert(result.message || 'Reprocess started. Refresh the claims list in a few minutes.')
      } else {
        alert(`Reprocess complete!\n\nProcessed: ${result.processed}\nCreated: ${result.created}\nUpdated: ${result.updated}\nSkipped: ${result.skipped}\nFailed: ${result.failed}\nTotal: ${result.total}`)
      }
      const [list, analytic, shots] = await Promise.all([
        pmFetchClaims(baseUrl, token, 1000),
        pmFetchClaimAnalytics(baseUrl, token),
        pmFetchScreenshots(baseUrl, token, 200)
      ])
      setClaims(Array.isArray(list) ? list : [])
      ...
      if (msg.includes('Failed to fetch') || msg.includes('NetworkError')) {
        alert('Network error: cannot reach the server. If you use a deployed backend (e.g. Railway), the request may have timed out or been blocked (e.g. CORS). Try again or refresh the page.')
      } else {
        alert('Failed to reprocess claims. See console for details.')
      }
    } finally {
  ...
  const refreshScreenshots = async () => {
    if (!baseUrl || !token) return
    try {
      const shots = await pmFetchScreenshots(baseUrl, token, 200)
      setScreenshots(Array.isArray(shots) ? shots : [])
    } catch (e) {
      ...
      <div className="flex flex-col gap-6 overflow-x-hidden min-w-0">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-400 mt-1">
              Structured claims from screenshots — de-duplicated and enriched. Extract claim data from any claims UI.
            </p>
          </div>
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={loading}
              className="flex items-center gap-2 px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors disabled:opacity-50"
              title="Reload claims from server (e.g. after running reprocess script)"
            >
            ...
            <button
              ...
              disabled={exporting}
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white transition-colors disabled:opacity-50"
            >
              <Download size={16} />
          ...
          {[
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-3 text-sm font-medium rounded-t-lg border-b-2 transition-colors ${
                activeTab === t.id
                  ? 'border-emerald-500 text-emerald-600 dark:text-emerald-400 bg-slate-50 dark:bg-slate-800/50'
        ...
        {activeTab === 'table' && (
          <div className="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden shadow-sm">
            <DataTable
              rows={rows}
              columns={columns}
              searchKeys={['claimId', 'reopened', 'providerName', 'patientName', 'memberId', 'status', 'claimType']}
                ...
                {loadingDetail ? (
                  <div className="grid gap-4 md:grid-cols-2 text-sm text-slate-700 dark:text-slate-300">
                    <div className="space-y-1 opacity-80">
                      <p><span className="font-medium text-slate-500 dark:text-slate-400">Status:</span> {selectedClaim?.status ?? '—'}</p>
                      <p><span className="font-medium text-slate-500 dark:text-slate-400">Type:</span> {cellValue(selectedClaim?.claimType, 30) || '—'}</p>
                      <p><span className="font-medium text-slate-500 dark:text-slate-400">Provider:</span> {selectedClaim?.providerName ?? '—'}</p>
                      <p><span className="font-medium text-slate-500 dark:text-slate-400">Patient:</span> {selectedClaim?.patientName ?? '—'}</p>
                    </div>
                    <div className="space-y-1 opacity-80">
                      <p><span className="font-medium text-slate-500 dark:text-slate-400">Member ID:</span> {selectedClaim?.memberId ?? '—'}</p>
                      <p><span className="font-medium text-slate-500 dark:text-slate-400">Received:</span> {selectedClaim?.receivedDate ? new Date(selectedClaim.receivedDate).toLocaleDateString() : '—'}</p>
                      <p><span className="font-medium text-slate-500 dark:text-slate-400">Processing:</span> {formatDuration(selectedClaim?.processingDurationSec ?? 0)}</p>
                    </div>
                    <p className="text-slate-500 dark:text-slate-400 col-span-2">Loading full details…</p>
                  </div>
                ) : claimDetail ? (
                  <div className="grid gap-4 md:grid-cols-2 text-sm text-slate-700 dark:text-slate-300">
                    <div className="space-y-1">
                      <p><span className="font-medium text-slate-500 dark:text-slate-400">Status:</span> {claimDetail.status || '—'}</p>
                      <p><span className="font-medium text-slate-500 dark:text-slate-400">Type:</span> {claimDetail.claimType || '—'}</p>
                      <p><span className="font-medium text-slate-500 dark:text-slate-400">Provider:</span> {claimDetail.providerName || '—'}</p>
                      <p><span className="font-medium text-slate-500 dark:text-slate-400">Patient:</span> {claimDetail.patientName || '—'}</p>
                    </div>
                    <div className="space-y-1">
                      <p><span className="font-medium text-slate-500 dark:text-slate-400">Member ID:</span> {claimDetail.memberId || '—'}</p>
                      <p><span className="font-medium text-slate-500 dark:text-slate-400">Received:</span> {claimDetail.receivedDate ? new Date(claimDetail.receivedDate).toLocaleDateString() : '—'}</p>
                      <p><span className="font-medium text-slate-500 dark:text-slate-400">Processing:</span> {formatDuration(claimDetail.processingDurationSec ?? 0)}</p>
                    </div>
                  </div>
                ) : selectedClaim ? (
                  <div className="grid gap-4 md:grid-cols-2 text-sm text-slate-700 dark:text-slate-300">
                  ...
                  <p className="text-slate-500 dark:text-slate-400">Click a row to load details or open the modal.</p>
                )}
              </div>
            )}
          </div>
          ...
          <div className="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl p-6">
            {screenshotItems.length > 0 ? (
              ...
              <div className="text-center py-12 text-slate-500 dark:text-slate-400">
                ...
                <p className="text-sm mt-1">Screenshots will appear here when captured by your team.</p>
              </div>
            )}
          </div>
        )}
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(selectedClaim.ocrTags || []).length ? (
                      selectedClaim.ocrTags.map(tag => (
                        <span key={tag} className="px-2 py-1 text-xs rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-700 dark:text-emerald-300">
                          {tag}
                        </span>
                      ))
                    ) : (
                      <p className="text-sm text-slate-500 dark:text-slate-400">No tags for this claim.</p>
                    )}
                  </div>
                </div>
                <div>
        ...
        {activeTab === 'analytics' && (
          <div className="space-y-6">
            {chartOverTime.length > 0 && (
              <div className="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl p-6">
                <h3 className="text-lg font-semibold mb-4 text-slate-800 dark:text-slate-100 flex items-center gap-2">
                  <Activity size={20} /> Claims over time
                </h3>
                <div className="h-56">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartOverTime}>
                      <defs>
                        <linearGradient id="claimOverTimeGrad" x1="0" y1="0" x2="0" y2="1">
                      ...
                      <Area type="monotone" dataKey="count" stroke="#10b981" fill="url(#claimOverTimeGrad)" strokeWidth={2} />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl p-6">
                ...
                <div className="h-64">
                  {chartByStatus.length > 0 ? (
                    ...
                    <div className="flex items-center justify-center h-full text-slate-500 dark:text-slate-400">No status data yet.</div>
                  )}
                </div>
              </div>
              <div className="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl p-6">
                <h3 className="text-lg font-semibold mb-4 text-slate-800 dark:text-slate-100 flex items-center gap-2">
                  <BarChart3 size={20} /> By Type
                </h3>
                <div className="h-64">
                  {chartByType.length > 0 ? (
                    ...
                    <div className="flex items-center justify-center h-full text-slate-500 dark:text-slate-400">No type data yet.</div>
                  )}
                </div>
              </div>
            </div>
            <div className="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl p-6">
              <h3 className="text-lg font-semibold mb-4 text-slate-800 dark:text-slate-100 flex items-center gap-2">
                <BarChart3 size={20} /> Top Providers
              </h3>
              <div className="h-72">
                {chartByProvider.length > 0 ? (
                  ...
                  <div className="flex items-center justify-center h-full text-slate-500 dark:text-slate-400">No provider data yet.</div>
                )}
              </div>
            </div>
          </div>
