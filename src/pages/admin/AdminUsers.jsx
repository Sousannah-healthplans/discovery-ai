...
import { adminFetchUserOverview, adminFetchUserSessions } from '../../lib/api';
import { BACKEND_URL } from '../../lib/config';

export default function AdminUsers() {
  const [users, setUsers] = useState([]);
  const [allUsers, setAllUsers] = useState([]); // Store all users for analytics
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`${baseUrl}/api/admin/analytics/users?limit=200`, {
      headers: {
      ...
      .then(res => {
        if (!res.ok) {
          throw new Error('Failed to fetch extension users');
        }
        return res.json();
      })
      .then(usersData => {
  ...
  const handleViewUser = async (user) => {
    ...
    try {
      const [overview, sessions] = await Promise.all([
        adminFetchUserOverview(baseUrl, user.userId, token),
        adminFetchUserSessions(baseUrl, user.userId, token, 100)
      ]);
  ...
  const usersByActivity = allUsers
    ...
    .slice(0, 10)
    .map(u => ({
      name: u.username || u.userId?.slice(0, 8) || 'Unknown',
      events: u.events || 0
    }));
  ...
  allUsers.forEach(u => {
    if (u.firstTs) {
      const date = new Date(u.firstTs).toISOString().slice(0, 10);
      usersByDate[date] = (usersByDate[date] || 0) + 1;
    }
  });
  const registrationData = Object.entries(usersByDate)
      ...
      <DashboardLayout variant="admin">
        <div className="flex items-center justify-center py-12">
          <div className="text-center">
            ...
            <p className="text-slate-600 dark:text-slate-400">Loading extension users...</p>
          </div>
        </div>
      </DashboardLayout>
    );
        ...
        {currentUsers.map((user, idx) => (
          <div key={user.userId || idx} className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:bg-white/10 transition-colors">
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="flex justify-between">
                <span className="text-slate-500 dark:text-slate-500">Activity span:</span>
                <span className="text-slate-600 dark:text-slate-400">
                  {formatDuration(user.firstTs, user.lastTs)}
                </span>
              </div>
            </div>
            ...
            <div className="mt-4 pt-3 border-t border-white/10">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-cyan-500/20 hover:bg-cyan-500/30 rounded-lg text-cyan-400 transition-colors text-sm"
              >
                <Eye size={14} />
                View Details
              </button>
            ...
            <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-700">
              <div>
                <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200">
                  {selectedUser.username || `User ${selectedUser.userId?.slice(0, 12)}`}
                </h2>
                {selectedUser.email && (
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white/5 border border-white/10 rounded-xl p-6">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      <div>
                        <div className="text-sm text-slate-500 dark:text-slate-400 mb-1">Username</div>
                        <div className="font-medium">{userDetails.user.username || 'N/A'}</div>
                      </div>
                      <div>
                        <div className="text-sm text-slate-500 dark:text-slate-400 mb-1">Email</div>
                        <div className="font-medium">{userDetails.user.email || 'N/A'}</div>
                      </div>
                      <div>
                        <div className="text-sm text-slate-500 dark:text-slate-400 mb-1">User ID</div>
                        <code className="bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded text-xs">{userDetails.user.userId}</code>
                      </div>
                      <div>
                        <div className="text-sm text-slate-500 dark:text-slate-400 mb-1">Total Events</div>
                        <div className="font-medium">{userDetails.user.events || 0}</div>
                      </div>
                      <div>
                        <div className="text-sm text-slate-500 dark:text-slate-400 mb-1">First Seen</div>
                        <div className="font-medium">{formatDate(userDetails.user.firstTs)}</div>
                      </div>
                      <div>
                        <div className="text-sm text-slate-500 dark:text-slate-400 mb-1">Last Seen</div>
                        <div className="font-medium">{formatDate(userDetails.user.lastTs)}</div>
                      </div>
                      <div>
                        <div className="text-sm text-slate-500 dark:text-slate-400 mb-1">Activity Span</div>
                        <div className="font-medium">{formatDuration(userDetails.user.firstTs, userDetails.user.lastTs)}</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-end">
                    <a
                      href={`/admin/clients/${userDetails.user.userId}`}
